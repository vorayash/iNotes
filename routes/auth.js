...
router.post('/resetcheck', async (req, res) => {
    ...
    try {
        ...
        const phonetoken = jwt.sign(data, JWT_SECRET);
        success=true;
        res.json({success,phonetoken});
    } catch (error) {
        console.error(error.message);
        res.status(500).send("Internal server error");
    }
...
router.post('/resetpass',resetpassauth, async (req, res) => {
    ...
    try {
        ...
        success=true;
        res.json({email:password.email,success});
